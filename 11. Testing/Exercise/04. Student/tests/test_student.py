...
class TestStudent(TestCase):
    ...
    def setUp(self) -> None:
        self.student = Student("TestStud1")
        self.student_with_course = Student("TestStud2", {"math": ["some note"]})

    def test_correct_initialization(self):
        self.assertEqual("TestStud1", self.student.name)
        self.assertEqual({}, self.student.courses)
        self.assertEqual({"math": ["some note"]}, self.student_with_course.courses)

    def test_enroll_and_add_notes_to_existing_courses(self):
        result = self.student_with_course.enroll("math", ['just algebra', "math is so fun"])
        self.assertEqual(
            ["some note", 'just algebra', "math is so fun"],
    ...
    def test_enroll_and_add_notes_to_none_existing_course_with_third_param_y(self):
    ...
    def test_add_notes_on_existing_course(self):
        ...
        self.assertEqual(["some note", 'x + y = z'],
                         self.student_with_course.courses["math"]
                         )
