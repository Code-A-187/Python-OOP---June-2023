...
class Zoo:
    ...
    def pay_workers(self) -> str:
        salary_budget = sum([w.salary for w in self.workers])

        if salary_budget <= self.__budget:
            self.__budget -= salary_budget
            return f"You payed your workers. They are happy. Budget left: {self.__budget}"

        return "You have no budget to pay your workers. They are unhappy"

    def tend_animals(self) -> str:
        tend_budget = sum([a.money_for_care for a in self.animals])
        if tend_budget <= self.__budget:
            self.__budget -= tend_budget
